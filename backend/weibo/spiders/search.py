    ...
    def parse_weibo(self, response):
        ...
        for sel in response.xpath("//div[@class='card-wrap']"):
            ...
            if info:
                ...
                # 改进 created_at 提取
                created_at_raw = sel.xpath('.//div[@class="from"]/a[1]/text()').extract_first()
                if created_at_raw:
                    created_at_raw = created_at_raw.replace(' ', '').replace('\n', '')
                    if '前' in created_at_raw:
                        created_at_raw = created_at_raw.split('前')[0]
                    try:
                        weibo['created_at'] = util.standardize_date(created_at_raw)
                    except ValueError as e:
                        self.logger.error(f"标准化 created_at 失败: {created_at_raw}, 错误: {e}")
                        continue  # 跳过该记录
                else:
                    self.logger.error(f"无法提取 created_at: {weibo['id']}")
                    continue  # 跳过该记录
                ...
                if retweet_sel and retweet_sel[0].xpath('.//div[@node-type="feed_list_forwardContent"]/a[1]'):
                    ...
                    retweet['bid'] = retweet_sel[0].xpath(
                        './/p[@class="from"]/a/@href').extract_first().split('/')[-1].split('?')[0]
                    ...
                    # 改进 retweet 的 created_at 提取
                    created_at_raw = retweet_sel[0].xpath('.//p[@class="from"]/a[1]/text()').extract_first()
                    if created_at_raw:
                        created_at_raw = created_at_raw.replace(' ', '').replace('\n', '')
                        if '前' in created_at_raw:
                            created_at_raw = created_at_raw.split('前')[0]
                        try:
                            retweet['created_at'] = util.standardize_date(created_at_raw)
                        except ValueError as e:
                            self.logger.error(f"标准化 retweet created_at 失败: {created_at_raw}, 错误: {e}")
                            continue  # 跳过该记录
                    else:
                        self.logger.error(f"无法提取 retweet created_at: {retweet['id']}")
                        continue  # 跳过该记录
                ...
                self.logger.info(f"提取微博: ID={weibo['id']}, created_at={weibo['created_at']}")
